//#region node_modules/.nitro/vite/services/ssr/assets/init-synth-CXvZTryU.js
var DAW_SESSION = [
...
var SOUND = {
	early: {
		...
		drums: ["Hats on offbeats. Clap a 16th after the kick. No wall of percussion.", "Fills: one snare roll in the last 4 of a build. Then silence into the drop."],
		bass: ["Reverse bass on the offbeat after every kick. Same sample reversed, or a short saw pluck with a reverse envelope.", "Quieter than the kick. Sidechain it so the kick punch is clean."],
		lead: {
		...
		mix: ["If reverse bass is right, the track works. Do not bury it in layers.", "Kick owns 60 Hz and a soft 3 kHz click. Lead etch sits at 2.5 kHz, not on the kick click."]
	},
	nustyle: {
		kick: [
		...
		drums: ["Clap after the kick. Open hats in the break, tighter in the drop.", "Ride or shaker only if the tok has space. Last 8 of the build: snare roll into a gap."],
		bass: ["Reverse bass still on the offbeat, a bit fatter than early. Sidechain to the tok.", "Optional sub under the kick — mute it if the kick tail already fills 50 Hz."],
		lead: {
		...
		mix: ["Tok click must read on a phone speaker (3–5 kHz). Lead etch lives just below that at ~2.8 kHz.", "Chords thinner than the lead. If the drop is a wall, the stabs have too much sustain or too little HP."]
	},
	euphoric: {
		kick: [
		...
		drums: ["Clap after kick. Hats that sparkle, not gabber rides.", "Build: snare roll last 4–8, crash on 1 of the drop. Keep percussion thin under the supersaw."],
		bass: ["Reverse bass, sidechained, slightly quieter than rawstyle.", "Same key as the kick. If the kick is pitched, the bass follows."],
		lead: {
	...
	rawstyle: {
		...
		drums: ["Clap after kick. Hats optional. Industrial ticks in the break.", "Fills are kickrolls, not pretty snares."],
		bass: ["Reverse bass still on the offbeat, darker and quieter under a loud kick.", "High-pass ~70 Hz if the kick tail owns the sub."],
		lead: {
			name: "Screech",
		...
		mix: ["Screech etch 3.2 kHz, kick click 4.5 kHz — notch one if they stack. Never boost both.", "Kickrolls in the last 4 of a 16. Then a gap, then the downbeat."]
	},
	rawphoric: {
		kick: ["Two kicks: a pitched / musical one for the identity, a rawer one for the drop. Switch at Drop B if needed.", "Tune the musical kick. Distort the drop kick. Same click if you can, so the tok stays familiar."],
		drums: ["Euphoric hats in the break, tighter in the drop. Clap after kick always.", "Build into the raw drop: snare roll + kickroll in the last 4."],
		bass: ["Reverse bass in both halves. Louder under the raw kick, polite under the pitched kick."],
		lead: {
		...
		mix: ["The switch is the record. Automate kick bus distortion, lead preset, and the etch band — not 20 extra tracks.", "Keep one clap and one reverse bass so the two halves feel like one track."]
	},
	xtraraw: {
		kick: [
		...
		drums: ["Almost no hats. Clap after kick if it still reads. Do not pretty it up."],
		bass: ["Skip classic reverse bass if the zaag already fills the offbeat. If you use it, keep it thin."],
		lead: {
		...
		mix: ["If the kick is not the loudest thing, start over.", "High-pass music bus at 150–200 Hz so the zaag kick owns the mid. Etch lives on the kick, not a lead layer."]
	},
	uptempo: {
		kick: [
		...
		drums: ["Offbeat hats or nothing. Clap after kick if there is space. Fills = kick rolls and stops."],
		bass: ["Optional gabber offbeat. If the kick is full-range, mute bass."],
		lead: {
		...
		mix: ["Release times scale with BPM. Sidechain shorter. Kick bus clipper is the loudness.", "Etch 3 kHz on the kick or the stab, not both. If it smears, shorten every envelope 20%."]
	}
};
function phraseSoundTips(genre, kind) {
	const s = SOUND[genre];
	if (kind === "drop" || kind === "dropB") return [
		{
			title: "Kick",
			steps: s.kick
		},
		{
			title: `Serum / Spire lead — ${s.lead.name}`,
			steps: serumLines(s.lead)
		},
		{
			title: "EQ / etch",
			steps: [s.lead.eq, s.mix[0]]
		}
	];
	if (kind === "breakdown" || kind === "break") return [{
		title: `Serum / Spire chords — ${s.chords.name}`,
		steps: serumLines(s.chords)
	}, {
		title: "EQ / etch",
		steps: [s.chords.eq, s.lead.eq]
	}];
	if (kind === "build") return [{
		title: "Drums",
		steps: ["Last 4–8: snare roll, then a gap. Optional kickroll on the last beat.", ...s.drums.slice(0, 1)]
	}, {
		title: "Serum",
		steps: ["Open lead/chord filter across the build. Same MIDI as the drop — do not write a new riff.", s.lead.midi]
	}];
	if (kind === "intro" || kind === "outro" || kind === "tease") return [{
		title: "Kick / drums",
		steps: ["Filtered or half-open kick. Clap optional. Hats only.", s.kick[0]]
	}, {
		title: "Bass",
		steps: s.bass
	}];
	return [{
		title: `Serum lead — ${s.lead.name}`,
		steps: serumLines(s.lead)
	}, {
		title: "EQ / etch",
		steps: [s.lead.eq]
...
//#endregion
export { SPIRE_FROM_SCRATCH as a, SOUND as i, EQ_ETCH as n, SPIRE_KNOBS as o, SERUM_FROM_SCRATCH as r, phraseSoundTips as s, DAW_SESSION as t };
